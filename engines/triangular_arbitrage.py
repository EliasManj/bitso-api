...
class CryptoEngineTriArbitrage(object):
            ...
            if self.open_orders:
                ...
                if opportunities:
                    ...
                    if not self.mock:
                        ...
                        """
                        self.alertsservice.email_alert(self.emailto, "Order Placed", body)
                        self.open_orders = True
                        n_of_trades += 1
                        time.sleep(300)
                    else:
    ...
    def get_bid_route(self, books, fees):
        ...
        # bid route
        bid_route = (1 / books[0]['ask']['price']) * books[1]['bid']['price'] * books[2]['bid']['price']
    ...
    def get_ask_route(self, books, fees):
        ...
        fee_factor3 = 1 - float(fees[self.tickerPairC]['taker_fee_decimal']) / 100
        # bid route
        ask_route = (1/books[2]['ask']['price']) / books[1]['ask']['price'] * books[0]['bid']['price']
    ...
    def get_max_amounts_ask_route(self, books):
        ...
        balances = grequests.map([self.engine.get_balance(tickers=[self.tickerA, self.tickerB, self.tickerC])])[
            0].parsed
        max_amount_eth_mxn = self.calculate_max_amount(books[0], balances, 'bid', 'sell')
    ...
    def get_max_amounts_bid_route(self, books):
        ...
        max_amount_eth_btc = self.calculate_max_amount(books[1], balances, 'bid', 'sell')
        max_amount_btc_mxn = self.calculate_max_amount(books[2], balances, 'bid', 'sell')
    ...
    def calculate_max_amount(self, order, balance, order_type, action):
        ...
        if action == 'buy':
            balance_amount_major = balance[ticker_right]*.8 / order[order_type]['price']
            amount_to_trade = min(balance_amount_major, order[order_type]['amount'])
        else:
            balance_amount_major = balance[ticker_left]*.8
            amount_to_trade = min(balance_amount_major, order[order_type]['amount'])
